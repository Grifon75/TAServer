...
public class RemoteService implements RemoteServiceRemote {
    ...
    @Override
    public Boolean addAvt(String login, String password) {
        List<Broker> brokers = brokerFacade.findAll();
        for(Broker broker:brokers){
            if(broker.getLogin().equals(login) && broker.getPassword().equals(password))
                return true;
        }
        return false;
    }
